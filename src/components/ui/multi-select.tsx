...
>(({ className, children, ...props }, ref) => {
  ...
  return (
    <div
      ref={ref}
      className={cn(
        'flex h-12 w-full items-center justify-between whitespace-nowrap rounded-md border border-gray-200 bg-transparent px-3 py-2 text-sm shadow-sm ring-offset-background placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-primary focus-visible:ring-primarydisabled:cursor-not-allowed disabled:opacity-50 [&>span]:line-clamp-1',
        className
      )}
      {...props}
    >
...
>(({ className, ...props }, ref) => {
  ...
  return (
    <CommandPrimitive.Input
      ...
      className={cn(
        "ml-2 bg-transparent outline-none  flex-1 font-normal px-1 placeholder:text-[#9d9d9d]",
        className,
...
const MultiSelectorItem = forwardRef<
  ...
  { value: string; isLoading?: boolean } & React.ComponentPropsWithoutRef<
    typeof CommandPrimitive.Item
  >
>(({ className, value, isLoading, children, ...props }, ref) => {
